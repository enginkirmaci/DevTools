...
public partial class SnapOverlayEditor : UserControl
{
    ...
    public LayoutOverlay GetOverlay()
    {
        ...
        {
            Point = new Tools.SnapIt.Common.Graphics.Point((float)Margin.Left, (float)Margin.Top),
            Size = new Tools.SnapIt.Common.Graphics.Size((float)Width, (float)Height),
            MiniOverlay = new LayoutOverlay
            {
                Point = new Tools.SnapIt.Common.Graphics.Point((float)(Margin.Left + MiniOverlay.Margin.Left), (float)(Margin.Top + MiniOverlay.Margin.Top)),
                Size = new Tools.SnapIt.Common.Graphics.Size((float)MiniOverlay.Width, (float)MiniOverlay.Height)
            }
        };
    }
    ...
    public void SetPos(Point point, Size size, LayoutOverlay miniOverlay)
    {
        ...
        if (miniOverlay != null)
        {
            SetPos(MiniOverlay, miniOverlay.Point.Convert(), miniOverlay.Size.Convert());
        }
        else
        {
            var factor = 0.3;
    ...
    protected override void OnMouseMove(MouseEventArgs e)
    {
        ...
        else
        {
            ...
            if (new_width > 0 && new_height > 0)
            {
                ...
                Dev.Log($"{selectedElement.Name}, {selectedElement.GetType()}");
