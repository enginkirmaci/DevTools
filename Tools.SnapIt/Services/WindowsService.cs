...
public partial class WindowsService : IWindowsService
{
			...
			foreach (var rule in matchRulesForMouse)
			{
				...
				switch (rule.MatchRule)
				{
					...
					case MatchRule.Wildcard:
						isMatched = WildcardMatch(rule.Keyword, Title);
						break;
				}
	...
	private static bool WildcardMatch(string pattern, string input)
	{
		...
		var regex = WildcardCache.GetOrAdd(pattern, p => new Regex(p, RegexOptions.IgnoreCase | RegexOptions.Compiled));
