...
public class SettingService : ISettingService
{
    ...
    public async Task<bool> GetStartupTaskStatusAsync()
    {
        ...
        //        try
        //        {
        //            var startupTask = await StartupTask.GetAsync("Tools.SnapItStartupTask"); // Pass the task ID you specified in the appxmanifest file
    ...
    public async Task SetStartupTaskStatusAsync(bool isActive)
    {
        ...
        //        try
        //        {
        //            var startupTask = await StartupTask.GetAsync("Tools.SnapItStartupTask");
        //            if (isActive)
        //            {
        ...
        using (var key = Microsoft.Win32.Registry.CurrentUser.OpenSubKey("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run", true))
        {
            if (isActive)
            {
                key.SetValue(Constants.AppRegistryKey, "\"" + Environment.ProcessPath + "\"");
            }
            else
            {
